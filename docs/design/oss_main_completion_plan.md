...
| **D5** | **The segmentation service.** The detection cascade's segmenter leg calls an HTTP service whose container was never ported. | (a) port and genericize it (~820 LOC); (b) make the segmenter leg optional and document that users supply their own; (c) drop the leg. | **(b) for this release, (a) later.** (b) is a day of work and makes the cascade honest; (a) is a meaningful project and the reference implementation is tied to a specific model family. **UPDATED — (a) has now landed too**: `docker/segmenter/` ships a SAM 3 service behind its own opt-in compose profile, with `text_prompt` required per request so the server carries no domain. (b) still holds underneath it: an empty `SAM3_URL` leaves the leg a clean no-op. |
