...
```bash
cd /path/to/openprocessor/benchmarks
```
...
```bash
./triton_bench \
  --images /data/REFERENCE_SAMPLE_PICTURES \
  --clients 64 \
  --duration 60 \
  --warmup 10 \
  --track all
...
```
--images string
    Directory containing test images (default: "/data/REFERENCE_SAMPLE_PICTURES")
...
```json
{
  ...
  "config": {
    "image_dir": "/data/REFERENCE_SAMPLE_PICTURES",
    "num_clients": 64,
    "duration": "60s",
    "warmup": 10
  },
...
- Check image directory exists: `ls /data/REFERENCE_SAMPLE_PICTURES`
