#!/usr/bin/env python3
"""
Test script for FAISS IVF clustering with reference sample images.

This script:
1. Creates OpenSearch indexes
2. Ingests images from reference sample folder
3. Trains FAISS clustering
4. Creates mosaic visualizations of clusters
5. Tests similarity search

Usage:
    python scripts/test_clustering.py --images /data/REFERENCE_SAMPLE_PICTURES --limit 200
"""
...
async def ingest_images(image_dir: Path, limit: int | None = None, concurrency: int = 64):
    ...
    async def ingest_with_semaphore(client: httpx.AsyncClient, image_path: Path):
        ...
        async with semaphore:
            try:
                image_id = f'reference_{image_path.stem}'
...
async def main():
    parser = argparse.ArgumentParser(description='Test FAISS clustering with reference images')
    parser.add_argument(
        '--images',
        type=Path,
        default=Path('/data/REFERENCE_SAMPLE_PICTURES'),
        help='Path to image directory',
    )
    parser.add_argument(
        '--output',
        type=Path,
        default=Path('/path/to/openprocessor/cluster_test_output'),
