...
Usage:
    ...
    python scripts/resize_images.py /mnt/nvm/REFERENCE_SAMPLE_PICTURES --size 1024
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=640,
        help="Maximum dimension in pixels (default: 640)"
    )
    parser.add_argument(
        "--output",
        type=str,
        default=None,
        help="Output directory (default: <input_dir>_<size>px)"
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    print(f"Quality: 100 (maximum, no compression artifacts)")
    print(f"Workers: {num_workers} processes")
    print()

    # Prepare arguments for multiprocessing
    task_args = [
        (img_file, output_dir / f"{img_file.stem}.jpg", args.size)
