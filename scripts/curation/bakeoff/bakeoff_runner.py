...
still accepted for back-compat::

    {
        ...
        'datasets': [
            ...
            {'name': 'andrewmvd', 'path': '/data/datasets/plates/andrewmvd_car_plate'},
        ],
        'verify_frozen': true,
...
# LEGACY_BAKEOFF_GPUS (comma-separated PCI ids; default the A6000 with headroom) and
# LEGACY_BAKEOFF_CONCURRENCY (parallel task slots).
_GPUS: list[str] = [
    g.strip() for g in os.environ.get('LEGACY_BAKEOFF_GPUS', '0').split(',') if g.strip()
]
_CONCURRENCY: int = max(1, int(os.environ.get('LEGACY_BAKEOFF_CONCURRENCY', '4')))
...
def _run_coreml_mac_leg(quant: dict[str, Any], out_dir: Path) -> None:
    ...
    Gated by the ``LEGACY_COREML_HOST`` env var (e.g. ``user@host.local``)
    ...
    """
    host = os.environ.get('LEGACY_COREML_HOST')
    if not host:
        print('[bakeoff] coreml: LEGACY_COREML_HOST not set, skipping Mac leg', flush=True)
        return
    repo_root = Path(__file__).resolve().parents[3]
    model_id = quant.get('model_id', 'ours_yolo26n')
    argv = [
