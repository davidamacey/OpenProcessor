    ...
    async def _on_failure(self, url: str) -> None:
        ...
        async with self._cb_lock:
            ...
            if state.half_open_in_flight:
                ...
                state.failures.clear()
                KB_SAM3_CIRCUIT_OPEN_TOTAL.labels(host=url).inc()
        ...
        for attempt in range(_MAX_ATTEMPTS):
            try:
                ...
                if attempt < _MAX_ATTEMPTS - 1:
                    ...
                    continue
                KB_SAM3_REQUEST_RETRIES_TOTAL.labels(
    ...
    def _record_timings(
        ...
        *,
        ...
        if t_end is not None and post_returned is not None:
            ...
            response = 0.0
        KB_SAM3_REQUEST_WAIT_SECONDS.labels(host=url, outcome=outcome).observe(wait)
        KB_SAM3_REQUEST_INFLIGHT_SECONDS.labels(host=url, outcome=outcome).observe(inflight)
        KB_SAM3_REQUEST_RESPONSE_SECONDS.labels(host=url, outcome=outcome).observe(response)
