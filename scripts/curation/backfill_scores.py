...
Requires ``LEGACY_SCORES_ENABLED=1`` (or ``--force``) — same feature-flag gate
...
async def _async_main(args: argparse.Namespace) -> int:
    ...
    if not _scores_enabled() and not args.force:
        logger.error('LEGACY_SCORES_ENABLED is not set — pass --force to backfill anyway')
        return 1
...
def main() -> int:
    ...
    p.add_argument(
        '--force',
        action='store_true',
        help='Backfill even if LEGACY_SCORES_ENABLED is unset.',
    )
