...
def test_single_label_writes_class_and_grows_history(
    kb: Any, opensearch: Any, label_cohort: list[str]
) -> None:
    crop_id = label_cohort[0]
    before = _source(opensearch, crop_id)
    history_before = len(before.get('class_id_history') or [])

    resp = kb.put(f'/crops/{crop_id}/label', json={'class_id': 3, 'label_source': 'human'})
...
def test_second_label_appends_another_history_entry(
    ...
    crop_id = label_cohort[0]
    before = _source(opensearch, crop_id)
    kb.put(f'/crops/{crop_id}/label', json={'class_id': 4}).raise_for_status()
    after = _source(opensearch, crop_id)
...
def test_unknown_class_is_rejected_and_leaves_the_doc_untouched(
    ...
    resp = kb.put(f'/crops/{crop_id}/label', json={'class_id': 9999})
...
def test_batch_label_updates_every_crop(kb: Any, opensearch: Any, label_cohort: list[str]) -> None:
    batch = label_cohort[2:8]
    resp = kb.put('/crops/batch_label', json={'crop_ids': batch, 'class_id': 5})
    assert resp.status_code == 200, resp.text
    body = resp.json()
...
def test_unlabel_resets_class_provenance(kb: Any, opensearch: Any, label_cohort: list[str]) -> None:
    crop_id = label_cohort[2]
    resp = kb.delete(f'/crops/{crop_id}/label')
    assert resp.status_code == 200, resp.text
...
def test_review_dismiss_is_recorded(kb: Any, opensearch: Any, label_cohort: list[str]) -> None:
    crop_id = label_cohort[9]
    resp = kb.post(f'/crops/{crop_id}/review_dismiss')
    assert resp.status_code == 200, resp.text
    src = _source(opensearch, crop_id)
...
def test_exclude_then_unexclude_round_trip(
    ...
    resp = kb.post('/crops/batch_exclude', json={'crop_ids': batch, 'reason': 'blurry'})
    assert resp.status_code == 200, resp.text
    ...
    resp = kb.post('/crops/batch_unexclude', json={'crop_ids': batch})
    assert resp.status_code == 200, resp.text
...
def test_move_relabels_into_the_destination_cluster(
    ...
    resp = kb.post('/crops/move', json={'crop_ids': batch, 'cluster_id': 6})
    assert resp.status_code == 200, resp.text
...
def test_flag_new_class_queues_for_the_curator(
    ...
    resp = kb.post('/crops/flag_new_class', json={'crop_ids': batch, 'note': 'live-harness probe'})
    assert resp.status_code == 200, resp.text
...
def test_parallel_labels_of_one_item_do_not_tear_the_history(
    ...
    def label(class_id: int) -> int:
        return kb.put(f'/crops/{crop_id}/label', json={'class_id': class_id}).status_code
