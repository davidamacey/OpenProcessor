...
def _classes(kb: Any) -> dict[int, dict[str, Any]]:
    resp = kb.get('/classes')
    resp.raise_for_status()
    return {c['class_id']: c for c in resp.json()['classes']}


def test_registry_lists_the_seeded_classes_with_live_counts(kb: Any) -> None:
    classes = _classes(kb)
...
def test_create_class_appends_to_the_registry(kb: Any) -> None:
    resp = kb.post('/classes', json={'name': 'live_probe_class', 'group': 'harness'})
    ...
    dup = kb.post('/classes', json={'name': 'live_probe_class'})
    assert dup.status_code == 409, dup.text


def test_rename_and_hotkey_update_persist(kb: Any) -> None:
    classes = _classes(kb)
    target = next(c for c in classes.values() if c['class_name'] == 'live_probe_class')
    class_id = target['class_id']

    resp = kb.put(f'/classes/{class_id}', json={'name': 'live_probe_renamed'})
    assert resp.status_code == 200, resp.text
    assert resp.json()['class_name'] == 'live_probe_renamed'

    resp = kb.put(f'/classes/{class_id}', json={'hotkey_letter': 'q'})
    assert resp.status_code == 200, resp.text
    assert resp.json()['hotkey_letter'] == 'q'

    # A reserved action letter must be refused rather than silently bound.
    resp = kb.put(f'/classes/{class_id}', json={'hotkey_letter': 'g'})
    assert resp.status_code == 422, resp.text

    assert _classes(kb)[class_id]['class_name'] == 'live_probe_renamed'


def test_sync_to_opensearch_mirrors_the_registry(kb: Any, opensearch: Any) -> None:
    resp = kb.post('/classes/sync_to_opensearch')
    assert resp.status_code == 200, resp.text
    body = resp.json()
...
def test_settings_put_then_get_round_trips(kb: Any) -> None:
    ...
    resp = kb.put('/settings', json={'defaults': {axis: chosen}})
    assert resp.status_code == 200, resp.text
    assert resp.json()['defaults'][axis] == chosen

    assert kb.get('/settings').json()['defaults'][axis] == chosen

    # An id that is not advertised for that axis must 422, not persist.
    bad = kb.put('/settings', json={'defaults': {axis: 'definitely_not_a_strategy'}})
    assert bad.status_code == 422, bad.text
    assert kb.get('/settings').json()['defaults'][axis] == chosen


def test_freeze_test_holdout_selects_a_deterministic_cohort(kb: Any, opensearch: Any) -> None:
    resp = kb.post('/test_holdout/freeze', json={'percent': 10})
    assert resp.status_code == 200, resp.text
    body = resp.json()
    ...
    stats = kb.get('/test_holdout/stats')
    ...
    again = kb.post('/test_holdout/freeze', json={'percent': 10})
...
def test_merge_is_refused_when_it_would_relabel_frozen_holdout_rows(
    ...
    resp = kb.post('/classes/merge', json={'source_id': FROZEN_CLASS_ID, 'target_id': 3})
    assert resp.status_code == 409, resp.text
    assert 'test_holdout' in resp.text

    # Refusal must be total: the registry entry stays active.
    assert _classes(kb)[FROZEN_CLASS_ID]['deprecated'] is False


def test_merge_success_relabels_items_and_confirmed_labels(kb: Any, opensearch: Any) -> None:
...
def test_labeling_against_a_deprecated_class_is_refused(kb: Any, opensearch: Any) -> None:
    crop_id = crop_ids_in(opensearch, 'noise', limit=1)[0]
    resp = kb.put(f'/crops/{crop_id}/label', json={'class_id': MERGE_SOURCE_CLASS_ID})
