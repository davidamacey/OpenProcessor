...
def test_preflight_reports_checks_and_writes_no_job_file(kb: Any) -> None:
    before = sorted(p.name for p in JOBS_DIR.glob('*.job.json'))

    resp = kb.post('/train/preflight', json=_spec())
...
def test_start_writes_a_job_file_and_stops_no_container(kb: Any, run_state: dict[str, Any]) -> None:
    containers_before = docker_ps_snapshot()

    resp = kb.post('/train/start', params={'force': True}, json=_spec())
...
def test_fake_trainer_drives_the_run_to_finished(kb: Any, run_state: dict[str, Any]) -> None:
    job_id = run_state.get('job_id')
    assert job_id, 'the start scenario must run first'
    paths = _job_files(job_id)

    state = wait_for_state(
        lambda: kb.get(f'/train/status/{job_id}').json(),
    ...
    latest = kb.get('/train/status')
    assert latest.status_code == 200, latest.text
    assert latest.json()['job_id'] == job_id

    runs = kb.get('/train/runs', params={'limit': 10})
    assert runs.status_code == 200, runs.text
    assert job_id in {item['job_id'] for item in runs.json()['items']}

    log = kb.get(f'/train/log/tail/{job_id}', params={'lines': 20})
    assert log.status_code == 200, log.text
    assert any(job_id in line for line in log.json()['lines']), log.json()


def test_manifest_is_served_for_a_finished_run(kb: Any, run_state: dict[str, Any]) -> None:
    job_id = run_state.get('job_id')
    assert job_id
    resp = kb.get(f'/train/manifest/{job_id}')
    assert resp.status_code == 200, resp.text
    assert resp.json()['job_id'] == job_id


def test_cancel_drops_the_sentinel_and_the_trainer_honours_it(kb: Any) -> None:
    started = kb.post('/train/start', params={'force': True}, json=_spec(model_size='s'))
    assert started.status_code == 201, started.text
    job_id = started.json()['job_id']
    paths = _job_files(job_id)

    resp = kb.post(f'/train/cancel/{job_id}')
    assert resp.status_code == 200, resp.text
...
def test_unknown_job_id_is_a_404(kb: Any) -> None:
    resp = kb.get('/train/status/2026-01-01T00-00-00_nosuchrun')
    assert resp.status_code == 404, resp.text


def test_campaign_submit_then_cancel_campaign(kb: Any) -> None:
    campaign_spec = {
        'dataset_export_dir': CONTAINER_EXPORT_CURRENT,
        'cuda_visible_devices': '0',
        'runs': [
    ...
    resp = kb.post('/train/start_campaign', params={'force': True}, json=campaign_spec)
    assert resp.status_code == 201, resp.text
    body = resp.json()
    ...
    resp = kb.post(f'/train/cancel_campaign/{campaign_id}')
    assert resp.status_code == 200, resp.text
...
def test_profiles_and_presets_are_served(kb: Any) -> None:
    profiles = kb.get('/train/profiles')
    assert profiles.status_code == 200, profiles.text
    assert profiles.json()['profiles'], profiles.text

    presets = kb.get('/train/presets')
