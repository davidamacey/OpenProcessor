...
def first_export(kb: Any) -> dict[str, Any]:
    resp = kb.post('/export/yolo', json={'version_tag': 'live-a', 'seed': 42})
...
def test_export_is_deterministic_across_two_tags(kb: Any, first_export: dict[str, Any]) -> None:
    resp = kb.post('/export/yolo', json={'version_tag': 'live-b', 'seed': 42})
    assert resp.status_code == 200, resp.text
    second = resp.json()
...
def test_export_status_and_dataset_listing_see_both_runs(kb: Any) -> None:
    status = kb.get('/export/status')
    assert status.status_code == 200, status.text
    assert status.json()['status'] == 'success'

    listing = kb.get('/export/datasets')
...
def test_registry_artifact_is_served_byte_for_byte(
    kb: Any, artifact: str, content_type: str
) -> None:
    current_dir = kb.get('/export/status').json()['export_dir']
    resp = kb.get(f'/export/registry/{artifact}')
    assert resp.status_code == 200, resp.text
...
def test_unknown_export_artifact_is_a_404(kb: Any) -> None:
