...
def test_refine_splits_a_candidate_cluster_into_subclusters(client: Any, opensearch: Any) -> None:
    resp = client.post(f'/clusters/refine/{CANDIDATE_CLUSTER_ID}')
...
def test_refine_is_idempotent(client: Any, opensearch: Any) -> None:
...
def test_refine_honours_the_member_floor(client: Any) -> None:
    resp = client.post(f'/clusters/refine/{NONEXISTENT_CLUSTER_ID}')
    assert resp.status_code == 200, resp.text
    body = resp.json()
...
def test_cluster_cards_report_kind_purity_and_subclusters(client: Any) -> None:
    resp = client.get('/clusters', params={'per_cluster': 2, 'max_clusters': 100})
    assert resp.status_code == 200, resp.text
    body = resp.json()
...
def test_auto_promote_dry_run_writes_nothing(client: Any, opensearch: Any) -> None:
    ...
    resp = client.post('/clusters/auto_promote', params={'dry_run': True})
    assert resp.status_code == 200, resp.text
    body = resp.json()
...
def test_auto_promote_apply_validates_only_the_high_purity_cluster(
    ...
    resp = client.post('/clusters/auto_promote')
    assert resp.status_code == 200, resp.text
...
def test_region_refine_writes_region_subids(client: Any, opensearch: Any) -> None:
    buckets = search(
        opensearch,
        INDEXES['items'],
        {
...
def _run_scorer(client: Any, scorer: str) -> dict[str, Any]:
    resp = client.post('/scores/compute', json={'scorers': [scorer]})
    assert resp.status_code == 200, resp.text
    state = wait_for_state(
        lambda: client.get('/scores/status').json(),
...
def test_scores_compute_writes_the_overlay_fields(client: Any, opensearch: Any) -> None:
    state = _run_scorer(client, 'mistakenness')
    ...
    coverage = client.get('/scores/coverage')
...
def test_embedding_scorers_report_their_missing_prerequisite(client: Any) -> None:
    ...
    """
    state = _run_scorer(client, 'uniqueness')
    assert state['status'] == 'failed', state
    assert 'IVF centroid store' in (state['error'] or ''), state
    assert state['processed'] == 0, state


def test_scores_cancel_on_an_idle_job_is_a_no_op(client: Any) -> None:
    resp = client.post('/scores/cancel')
    assert resp.status_code == 200, resp.text
    body = resp.json()
...
def test_unknown_scorer_is_rejected(client: Any) -> None:
    resp = client.post('/scores/compute', json={'scorers': ['not_a_scorer']})
    assert resp.status_code == 400, resp.text
    assert 'unknown scorer' in resp.text


def test_diverse_select_returns_an_ordering_and_mutates_nothing(client: Any, opensearch: Any) -> None:
...
def test_viz_projection_rebuild_then_serve(client: Any, opensearch: Any) -> None:
    resp = client.post('/viz/projection/rebuild', params={'scope': 'residual'})
    assert resp.status_code == 202, resp.text

    state = wait_for_state(
        lambda: client.get('/viz/projection/status').json(),
        lambda s: s.get('status') != 'running',
        timeout=600.0,
        interval=2.0,
    )
    ...
    served = client.get('/viz/projection', params={'max_points': 1000})
