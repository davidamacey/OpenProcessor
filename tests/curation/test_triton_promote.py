...
async def test_promote_does_not_rmtree_before_copy(
    ...
    triton_name = 'kb_vehicle_smoke_v1'
...
async def test_promote_fresh_model_uses_version_1(
    ...
    assert (scratch_models_dir / 'kb_vehicle_smoke_v1' / '1' / 'model.onnx').is_file()
...
async def test_promote_without_overwrite_still_conflicts(
    ...
    model_dir = scratch_models_dir / 'kb_vehicle_smoke_v1'
...
async def test_promote_copies_class_remap_into_model_dir(
    ...
    result = await promoter.promote(
        status=fake_status,
        triton_name='kb_vehicle_smoke_remap',
        class_id_to_name={0: 'pickup', 1: 'license_plate'},
        class_remap=remap,
    )
    assert result.class_remap_source == 'manifest'
    model_dir = scratch_models_dir / 'kb_vehicle_smoke_remap'
...
def _make_promoted_model_dir(scratch_models_dir: Path, name: str = 'kb_smoke_unload_v1') -> Path:
