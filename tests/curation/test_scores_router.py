...
Every test redirects ``KB_SCORES_STATE_DIR`` to a per-test ``tmp_path`` so
nothing touches the real ``/jobs/scores`` volume (mirrors
``test_train_jobs.py``'s ``KB_TRAIN_JOBS_DIR`` convention).
"""
...
def app_client(monkeypatch: pytest.MonkeyPatch, tmp_path) -> TestClient:
    ...
    monkeypatch.setenv('KB_SCORES_STATE_DIR', str(tmp_path / 'scores'))
