...
def _without_scripts(query: dict[str, Any]) -> dict[str, Any]:
    """Drop painless ``script`` clauses (the fake can't run painless; the
    normalizer's ``cluster_id != class_id`` script is re-checked below)."""
    q = copy.deepcopy(query)
    b = q.get('bool')
    if isinstance(b, dict):
        for key in ('must', 'filter'):
            if isinstance(b.get(key), list):
                b[key] = [c for c in b[key] if 'script' not in c]
    return q
...
class _NormalizingOpenSearch(QueryFakeOpenSearch):
    ...
    async def update_by_query(self, *, index: str, body: dict[str, Any], **_kw: Any) -> Any:
        ...
        updated = 0
        query = _without_scripts(body['query'])
        for doc in self.docs(index).values():
            if not matches(doc, query) or doc.get('class_excluded'):
                continue
