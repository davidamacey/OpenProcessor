...
@pytest.mark.parametrize('tab', sorted(LEGACY_TAB_CLAUSES))
def test_build_sort_none_is_byte_identical_to_legacy_clause(tab: str) -> None:
    ...
    clause, applied_id, fallback_reason = review_sorts.build_sort(None, tab=tab)
...
@pytest.mark.parametrize('tab', sorted(LEGACY_TAB_CLAUSES))
def test_build_sort_default_literal_matches_none(tab: str) -> None:
    """?sort=default must behave identically to omitting ?sort."""
    via_none = review_sorts.build_sort(None, tab=tab)
    via_default = review_sorts.build_sort('default', tab=tab)
...
def test_stable_sorts_resolve_to_their_own_clause(sort_id: str) -> None:
    registry = review_sorts.get_review_sorts()
    expected = registry[sort_id]
    assert expected.status == 'stable'
    clause, applied_id, fallback_reason = review_sorts.build_sort(sort_id, tab='all')
...
def test_uniqueness_is_shadow_and_not_selectable() -> None:
    registry = review_sorts.get_review_sorts()
    assert registry['uniqueness'].status == 'shadow'
    with pytest.raises(ValueError, match='not selectable'):
        review_sorts.build_sort('uniqueness', tab='all')


def test_unknown_sort_id_raises() -> None:
    with pytest.raises(ValueError, match='unknown review sort'):
        review_sorts.build_sort('not-a-real-sort', tab='all')
...
def test_mistakenness_disabled_by_default(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ValueError, match='not selectable'):
        review_sorts.build_sort('mistakenness', tab='all')


def test_mistakenness_promoted_to_experimental_when_enabled_and_shadow(
