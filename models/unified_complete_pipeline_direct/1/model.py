#!/usr/bin/env python3
"""
Triton Python Backend: Unified Complete Analysis Pipeline (Direct Tensor Input)

Complete GPU-accelerated pipeline accepting CPU-preprocessed tensors:
1. YOLO object detection (from preprocessed 640x640 tensor)
2. MobileCLIP global image embedding (from preprocessed 256x256 tensor)
3. MobileCLIP per-box embeddings (from original HD tensor)
...
- No DALI preprocessing - tensors are already preprocessed on CPU client-side
- Direct passthrough to YOLO and MobileCLIP models
...
Architecture:
    Inputs:
        - yolo_images: FP32 [3, 640, 640] - YOLO letterboxed input
        - clip_images: FP32 [3, 256, 256] - MobileCLIP preprocessed input
        - original_images: FP32 [3, H, W] - Full-res normalized image for crops
        - affine_matrices: FP32 [2, 3] - YOLO letterbox transformation
        - face_model: STRING "scrfd" or "yolo11" (default: "scrfd")

    Processing:
        1. Call YOLO model directly with yolo_images
        2. Call MobileCLIP image encoder directly with clip_images
        3. Call unified_embedding_extractor for per-box embeddings + face detection
        4. Run OCR on original_images
...
class TritonPythonModel:
    """Unified Complete Analysis Pipeline with Direct Tensor Input"""
    ...
    def initialize(self, args):
        ...
        logger.info(f'Initialized unified_complete_pipeline_direct (device={self.device})')
        logger.info('  Input: CPU-preprocessed tensors (no DALI)')
    ...
    def _process_request(self, request):
        # Get preprocessed tensor inputs
        yolo_images = pb_utils.get_input_tensor_by_name(request, 'yolo_images')
        clip_images = pb_utils.get_input_tensor_by_name(request, 'clip_images')
        original_images = pb_utils.get_input_tensor_by_name(request, 'original_images')
        ...
        logger.info(f'Processing with face_model={face_model} (direct tensor input)')

        if face_model == 'yolo11':
            return self._process_with_yolo11_face(
                yolo_images, clip_images, original_images, affine_matrices
            )
        else:
            return self._process_with_scrfd(
                yolo_images, clip_images, original_images, affine_matrices
            )

    def _process_with_scrfd(self, yolo_images, clip_images, original_images, affine_matrices):
        ...
        clip_images_np = self._triton_to_numpy(clip_images)
        original_images_np = self._triton_to_numpy(original_images)
        affine_matrices_np = self._triton_to_numpy(affine_matrices)

        # Ensure batch dimension
        if yolo_images_np.ndim == 3:
            yolo_images_np = yolo_images_np[np.newaxis]
        if clip_images_np.ndim == 3:
            clip_images_np = clip_images_np[np.newaxis]
        if original_images_np.ndim == 3:
            original_images_np = original_images_np[np.newaxis]
        if affine_matrices_np.ndim == 2:
            affine_matrices_np = affine_matrices_np[np.newaxis]

        # =================================================================
        # Step 1: YOLO detection (direct call with preprocessed tensor)
        # =================================================================
        yolo_request = pb_utils.InferenceRequest(
            ...
            inputs=[pb_utils.Tensor('images', yolo_images_np.astype(np.float32))]
        )
        yolo_response = yolo_request.exec()

        if yolo_response.has_error():
            raise RuntimeError(f'YOLO inference failed: {yolo_response.error().message()}')

        num_dets = self._triton_to_numpy(
            pb_utils.get_output_tensor_by_name(yolo_response, 'num_dets')
        )
        det_boxes = self._triton_to_numpy(
            pb_utils.get_output_tensor_by_name(yolo_response, 'det_boxes')
        )
        det_scores = self._triton_to_numpy(
            pb_utils.get_output_tensor_by_name(yolo_response, 'det_scores')
        )
        det_classes = self._triton_to_numpy(
            pb_utils.get_output_tensor_by_name(yolo_response, 'det_classes')
        )

        # =================================================================
        # Step 2: MobileCLIP global embedding (direct call with preprocessed tensor)
        # =================================================================
        clip_request = pb_utils.InferenceRequest(
            model_name='mobileclip2_s2_image_encoder',
            requested_output_names=['image_embeddings'],
            inputs=[pb_utils.Tensor('images', clip_images_np.astype(np.float32))]
        )
        clip_response = clip_request.exec()

        if clip_response.has_error():
            raise RuntimeError(f'MobileCLIP inference failed: {clip_response.error().message()}')

        global_embeddings = self._triton_to_numpy(
            pb_utils.get_output_tensor_by_name(clip_response, 'image_embeddings')
        )

        # =================================================================
        # Step 3: Unified embedding extractor (per-box + faces)
        # =================================================================
        extractor_request = pb_utils.InferenceRequest(
            model_name='unified_embedding_extractor',
            requested_output_names=[
                'box_embeddings', 'normalized_boxes',
                'num_faces', 'face_embeddings', 'face_boxes',
                'face_landmarks', 'face_scores', 'face_person_idx'
            ],
            inputs=[
                pb_utils.Tensor('original_image', original_images_np.astype(np.float32)),
                pb_utils.Tensor('det_boxes', det_boxes.astype(np.float32)),
                pb_utils.Tensor('det_classes', det_classes.astype(np.int32)),
                pb_utils.Tensor('det_scores', det_scores.astype(np.float32)),
                pb_utils.Tensor('num_dets', num_dets.astype(np.int32)),
                pb_utils.Tensor('affine_matrix', affine_matrices_np.astype(np.float32))
            ]
        )
        extractor_response = extractor_request.exec()

        if extractor_response.has_error():
            raise RuntimeError(f'unified_embedding_extractor failed: {extractor_response.error().message()}')

        box_embeddings = self._triton_to_numpy(
            pb_utils.get_output_tensor_by_name(extractor_response, 'box_embeddings')
        )
        normalized_boxes = self._triton_to_numpy(
            pb_utils.get_output_tensor_by_name(extractor_response, 'normalized_boxes')
        )
        num_faces = self._triton_to_numpy(
            pb_utils.get_output_tensor_by_name(extractor_response, 'num_faces')
        )
        face_embeddings = self._triton_to_numpy(
            pb_utils.get_output_tensor_by_name(extractor_response, 'face_embeddings')
        )
        face_boxes = self._triton_to_numpy(
            pb_utils.get_output_tensor_by_name(extractor_response, 'face_boxes')
        )
        face_landmarks = self._triton_to_numpy(
            pb_utils.get_output_tensor_by_name(extractor_response, 'face_landmarks')
        )
        face_scores = self._triton_to_numpy(
            pb_utils.get_output_tensor_by_name(extractor_response, 'face_scores')
        )
        face_person_idx = self._triton_to_numpy(
            pb_utils.get_output_tensor_by_name(extractor_response, 'face_person_idx')
        )
        ...
        # =================================================================
        # Step 4: Run OCR (from original_images tensor)
        # =================================================================
        ocr_result = self._run_ocr_from_tensor(original_images_np)

        return self._create_response(result, ocr_result, face_model='scrfd')

    def _process_with_yolo11_face(self, yolo_images, clip_images, original_images, affine_matrices):
        ...
        # =================================================================
        # Step 3: Per-box embeddings (without face detection)
        # =================================================================
        box_extractor_request = pb_utils.InferenceRequest(
            model_name='box_embedding_extractor',
            requested_output_names=['box_embeddings', 'normalized_boxes'],
            inputs=[
                pb_utils.Tensor('original_image', original_images_np.astype(np.float32)),
                pb_utils.Tensor('det_boxes', det_boxes.astype(np.float32)),
                pb_utils.Tensor('num_dets', num_dets.astype(np.int32)),
        ...
        box_extractor_response = box_extractor_request.exec()

        if box_extractor_response.has_error():
            raise RuntimeError(f'box_embedding_extractor failed: {box_extractor_response.error().message()}')

        box_embeddings = self._triton_to_numpy(
            pb_utils.get_output_tensor_by_name(box_extractor_response, 'box_embeddings')
        )
        normalized_boxes = self._triton_to_numpy(
            pb_utils.get_output_tensor_by_name(box_extractor_response, 'normalized_boxes')
        )

        # Get original shape from HD image
        if original_images_np.ndim == 4:
            _, _, orig_h, orig_w = original_images_np.shape
        else:
            _, orig_h, orig_w = original_images_np.shape

        orig_shape = np.array([orig_h, orig_w], dtype=np.int32)

        # Get affine matrix for coordinate transformation
        affine_matrix_for_face = affine_matrices_np[0] if affine_matrices_np.ndim == 3 else affine_matrices_np

        # =================================================================
        # Step 4: YOLO11-face detection on full image
        # =================================================================
        face_request = pb_utils.InferenceRequest(
            ...
            inputs=[
                pb_utils.Tensor('face_images', yolo_images_np.astype(np.float32)),
                pb_utils.Tensor('original_image', original_images_np.astype(np.float32)),
                pb_utils.Tensor('orig_shape', orig_shape[np.newaxis]),
                pb_utils.Tensor('affine_matrix', affine_matrix_for_face[np.newaxis])
            ]
        )
        ...
        if face_response.has_error():
            ...
            # Create output arrays with correct shape for unified_complete_pipeline_direct (64 max)
            face_embeddings = np.zeros((1, self.max_faces, 512), dtype=np.float32)
            face_boxes = np.zeros((1, self.max_faces, 4), dtype=np.float32)
            face_landmarks = np.zeros((1, self.max_faces, 10), dtype=np.float32)
            face_scores = np.zeros((1, self.max_faces), dtype=np.float32)
        ...
        # =================================================================
        # Step 5: Run OCR (from original_images tensor)
        # =================================================================
        ocr_result = self._run_ocr_from_tensor(original_images_np)

        return self._create_response(result, ocr_result, face_model='yolo11')

    def _run_ocr_from_tensor(self, original_images_np: np.ndarray) -> dict:
        """Run OCR detection and recognition from preprocessed tensor.

        The original_images tensor is in CHW format with values in [0, 1] range.
        We need to convert it back to BGR uint8 for OCR processing.
        """
        if self.ctc_decode is None:
            return self._empty_ocr_result()

        try:
            # Get image from tensor (handle batch dimension)
            if original_images_np.ndim == 4:
                img_tensor = original_images_np[0]  # Remove batch dim: [1, 3, H, W] -> [3, H, W]
            else:
                img_tensor = original_images_np

            # Convert CHW [0, 1] float32 to HWC BGR uint8
            # img_tensor is in RGB format, normalized to [0, 1]
    ...
    def finalize(self):
        logger.info('Finalizing unified_complete_pipeline_direct')
