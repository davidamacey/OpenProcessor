...
async def occ_skip_on_conflict_bulk(
    ...
    *,
    ...
    - Each bulk response item is classified: 200/201 -> updated;
      ...
      ``kb_worker_skip_human_won`` event, pulling
    ...
    - A ``client.bulk()`` failure (transport/connection error) degrades
      ...
      (kb_gemma.py, kb_classes.py, kb_auto_promote.py, bulk_writer.py),
...
def _is_human_marker(value: Any) -> bool:
    ...
    ``gemma_human_confirmed`` (kb_crops, kb_plates, kb_clustering).
...
async def occ_upsert_bulk(
    ...
    *,
    ...
    3. Present, no human-write field set on the existing doc → OCC update
       ...
       ``KB_INGEST_OCC_FINAL_CONFLICT`` and the doc is skipped.
    4. Present, at least one human-guard field set → client-side merge:
       ...
       the new value. Increments ``KB_INGEST_PRESERVED_HUMAN_LABEL``
        ...
        while True:
            try:
                ...
                for field in preserved_fields:
                    ...
                    KB_INGEST_PRESERVED_HUMAN_LABEL.labels(field=field).inc()
                ...
                if not is_conflict:
                    ...
                    break
                if attempt >= max_retries:
                    KB_INGEST_OCC_FINAL_CONFLICT.inc()
                    result['final_conflicts'] += 1
                    logger.info(
                        'kb_ingest_upsert_final_conflict',
                        doc_id=doc_id,
                        writer_id=writer_id,
                    )
                    break
                ...
                try:
                    ...
                    logger.warning(
                        'kb_ingest_upsert_refetch_failed',
                        doc_id=doc_id,
                        error=str(fetch_exc),
                    )
                    break
