...
Ported from the reference ``kb_plates_fp.py`` (310 LOC). Split out of
...
async def refine_plate_cluster_endpoint(
    ...
    try:
        ...
        logger.error('kb_plate_refine_failed', cluster_id=cluster_id, error=str(exc))
...
async def list_plate_clusters(
    ...
    """Cluster cards for the region buckets (mirrors /kb/clusters' shape).
...
async def suspected_false_positives(
    ...
    if scroll_id:
        try:
            ...
            logger.debug('kb_suspected_fp_clear_scroll_failed', error=str(exc))
