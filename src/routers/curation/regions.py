...
Ported from the reference ``kb_plates.py`` (622 LOC). Covers browsing
...
async def batch_set_crop_plate(
    ...
    for crop_id in payload.crop_ids:
        try:
            ...
            logger.warning('kb_batch_plate_update_failed', crop_id=crop_id, error=str(exc))
...
async def batch_set_plate_status(
    ...
    for crop_id in payload.crop_ids:
        try:
            ...
            logger.warning('kb_batch_plate_status_failed', crop_id=crop_id, error=str(exc))
            conflicts.append({'crop_id': crop_id, 'current_source': None})
    if updated:
        try:
            await opensearch.indices.refresh(index=CURATION_ITEMS_INDEX)
        except Exception as exc:
            logger.debug('kb_batch_plate_status_refresh_failed', error=str(exc))
    return {'updated': updated, 'conflicts': conflicts}
