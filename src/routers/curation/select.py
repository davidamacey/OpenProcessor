...
``KB_SELECT_DIVERSE_ENABLED`` alone.

Gated end-to-end by ``KB_SELECT_DIVERSE_ENABLED`` (default off, inline
...
1. ``GET /curation/crops?order=diverse`` needs a **full ranking of the whole
   ...
   inline pool cap than ``KB_SELECT_MAX_N`` (20,000, the *fetch* cap that
   mirrors ``cluster_outliers._MAX_MEMBERS``'s precedent). Solving
   ``n^2 <= KB_SELECT_SYNC_MAX_OPS`` for ``n`` gives the GET-only inline
...
2. ``POST /curation/select/diverse`` is user-``k``-bounded (O(n*k*d), linear in
   ...
   request. ``KB_SELECT_SYNC_MAX_OPS`` (default 3,000,000 — derived from
   ...
   ``n_pool * k <= KB_SELECT_SYNC_MAX_OPS`` (and the pool itself fit under
   ``KB_SELECT_MAX_N``, the fetch cap), answer inline with the documented
   ...
   pool uncapped by ``KB_SELECT_MAX_N`` (bounded only by the much larger
   ``KB_SELECT_JOB_MAX_N``, default 200,000 — a generous safety ceiling
...
table lists a single ``KB_SELECT_MAX_N=20000``. This module treats that
...
POST) and introduces ``KB_SELECT_SYNC_MAX_OPS``/``KB_SELECT_JOB_MAX_N`` as
...
def _max_n() -> int:
    try:
        return max(1, int(os.environ.get('KB_SELECT_MAX_N', str(DEFAULT_MAX_N))))
    except ValueError:
...
def _sync_max_ops() -> int:
    try:
        return max(1, int(os.environ.get('KB_SELECT_SYNC_MAX_OPS', str(DEFAULT_SYNC_MAX_OPS))))
    except ValueError:
...
def _job_max_n() -> int:
    try:
        return max(1, int(os.environ.get('KB_SELECT_JOB_MAX_N', str(DEFAULT_JOB_MAX_N))))
    except ValueError:
...
_CACHE_TTL_S = float(os.getenv('KB_SELECT_CACHE_TTL_S', '600'))
...
async def select_diverse(
    ...
    the scoped pool is small enough per ``KB_SELECT_SYNC_MAX_OPS``;
    ...
    if not _diverse_enabled():
        raise HTTPException(
            status_code=400,
            detail='diversity selection is disabled (set KB_SELECT_DIVERSE_ENABLED=1 to enable)',
        )
