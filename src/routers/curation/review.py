...
async def review_unmatched_terms(
    ...
    :py:class:`KbClassEntry` entries (or new ``SYNONYMS`` mappings if the
...
async def review_queue(
    ...
    for h in hits:
        ...
        items.append(
            {
                # KbCrop fields the labeler ReviewItem extends.
                'id': crop_id,
                'crop_id': crop_id,
...
async def freeze_test_holdout(
    ...
    durable freeze record under ``KB_STATE_DIR/test_holdout/`` so a bad
    ...
    if not force:
        try:
            ...
            logger.warning('kb_test_holdout_count_failed', error=str(exc))
