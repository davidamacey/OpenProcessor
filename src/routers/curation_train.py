...
# Disk-space threshold (design §15.1: ≥50 GB free on /mnt/nvm).
MIN_FREE_DISK_GB = 50
...
async def _count_validated_per_class(
    ...
    Uses the ``kb_vehicle_crops`` index. Validated crops are those with
    ``label_state == 'confirmed'``. We issue one bool-filter aggregation
    rather than N count requests.
    """
    if not class_ids:
        return {}
    # The kb_vehicle_crops schema uses a boolean ``label_validated`` field
...
def _resolve_disk_check_path(spec: TrainJobSpec) -> str:
    ...
    Previously hardcoded to ``/mnt/nvm`` — inside the yolo-api container,
    only specific subpaths under ``/mnt/nvm`` (e.g.
    a deployment-specific training-data mount, see the deployment's own
    compose overlay)
    are bind-mounted from the real training-data volume; ``/mnt/nvm``
...
async def _resolve_full_registry_for_promote(job_id: str) -> dict[int, str]:
    ...
    if snapshot_path:
        try:
            ...
            logger.warning(
                'kb_promote_registry_pin_unreadable',
                job_id=job_id,
            ...
            if pinned:
                return pinned
            logger.warning(
                'kb_promote_registry_pin_empty', job_id=job_id, snapshot_path=snapshot_path
            )
    else:
        logger.warning(
            'kb_promote_registry_pin_missing',
            job_id=job_id,
            note=(
...
async def promote_run(
    ...
    if class_remap.source == 'none' and is_subset_run:
        ...
        logger.warning(
            'kb_promote_class_remap_missing_force_bypass',
            job_id=job_id,
