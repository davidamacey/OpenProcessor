    ...
    *,
    ...
    if scroll_id:
        try:
            ...
            logger.info('kb_auto_promote_clear_scroll_failed', error=str(exc))
    return ids
    ...
    *,
    ...
    for bucket in resp.get('aggregations', {}).get('clusters', {}).get('buckets', []):
        ...
        try:
            ...
            logger.warning('kb_auto_promote_cluster_failed', cluster_id=cluster_id, error=str(exc))
            total_skipped += members
            continue
        if not doc_ids:
            continue
        ...
        def _merge_promote(_doc_id: str, current: dict[str, Any]) -> dict[str, Any]:
            ...
            # per-doc OCC bulk pass (same shape as kb_gemma.py's
        ...
        if result.get('errors'):
            logger.warning(
                'kb_auto_promote_bulk_partial_errors',
                cluster_id=cluster_id,
