    ...
    *,
    ...
    if scroll_id:
        try:
            ...
            logger.warning('kb_clear_scroll_failed', error=str(e))
    return members
...
async def _bulk_update_subids(
    ...
    *,
    ...
    if resp.get('errors'):
        logger.warning(
            'kb_bulk_update_subids_partial_errors',
            n_items=len(updates),
...
async def refine_cluster(
    ...
    *,
    ...
    log.info('kb_refine_cluster_start')
    ...
    if n_members < MIN_REFINE_MEMBERS:
        log.info(
            'kb_refine_cluster_skipped_too_small',
            n_members=n_members,
    ...
    if n_members > max_members:
        log.warning(
            'kb_refine_cluster_skipped_too_large',
            n_members=n_members,
    ...
    try:
        ...
        log.error('kb_refine_cluster_embedding_load_failed', error=str(e))
        raise
    ...
    log.info('kb_refine_cluster_done', **summary)
    return summary
...
# so existing callers (kb_pipeline, tests/test_kb_clustering) keep
...
async def assign_cluster_to_crop(
    ...
    """Convenience wrapper: assign a single embedding to its kb_vehicles cluster."""
...
async def should_retrain_centroids(client: AsyncOpenSearch) -> dict[str, Any]:
    ...
    idle-worker trigger (``kb_auto_label_worker.py``) always requests
...
async def _park_gated_residuals(
    ...
    *,
    ...
    try:
        ...
        # on a large kb_vehicle_crops query can fail client-side response
        ...
        logger.warning('kb_park_gated_residuals_failed', error=str(exc))
        return 0
...
async def cluster_residuals(
    ...
    * ``"hdbscan"`` — cuML GPU HDBSCAN; dormant. Collapses on this data
    ...
    if gate_active:
        ...
        if not coverage['sufficient']:
            return {
                ...
                'hint': (
                    'Run kb_backfill_crop_rank.py and kb_backfill_blur.py over the '
    ...
    for start in range(0, len(pairs), BULK_CHUNK):
        ...
        if br.get('errors'):
            logger.warning('kb_cluster_bulk_partial_errors', chunk_start=start)
        n_written += len(chunk)
        if progress is not None:
    ...
    if pairs:
        try:
            ...
            logger.debug('kb_cluster_refresh_failed', error=str(exc))
    ...
    try:
        ...
        logger.warning('kb_cluster_save_gate_failed', error=str(exc))
    ...
    *,
    ...
    if progress is not None:
        try:
            ...
            logger.debug('kb_ivf_assign_count_failed', error=str(exc))
    ...
    try:
        while True:
            ...
            if chunk_embs:
                ...
                br = await client.bulk(body=bulk_body, refresh=False)
                if br.get('errors'):
                    logger.warning('kb_ivf_assign_bulk_partial_errors', chunk_start=n_written)
        ...
        if scroll_id:
            try:
                ...
                logger.debug('kb_ivf_assign_clear_scroll_failed', error=str(exc))

    if n_written:
        try:
            await client.indices.refresh(index=ITEMS_INDEX)
        except Exception as exc:
            logger.debug('kb_ivf_assign_refresh_failed', error=str(exc))
...
async def cluster_region_residuals(
    ...
    *,
    ...
    try:
        ...
        logger.debug('kb_region_cluster_refresh_failed', error=str(exc))

    logger.info(
        'kb_cluster_region_residuals_done', n_regions=n, n_clusters=int(k), assigned=n_written
    )
    return {
        'status': 'success',
...
def _write_region_cluster_job(state: dict[str, Any]) -> None:
    try:
        ...
        logger.warning('kb_plate_cluster_job_write_failed', error=str(exc))
...
def mark_region_refine(cluster_id: int) -> None:
    ...
    try:
        ...
        logger.warning('kb_plate_refine_marker_write_failed', error=str(exc))
...
def _write_region_partition_marker(fp_count: int) -> None:
    try:
        ...
        logger.warning('kb_plate_partition_marker_write_failed', error=str(exc))
...
async def _count_false_positives(client: AsyncOpenSearch) -> int:
    ...
    try:
        ...
        logger.warning('kb_count_fp_failed', error=str(exc))
        return 0
        ...
        try:
            if rebuild_fp_centroids:
                try:
                    ...
                    logger.error('kb_plate_job_fp_build_failed', error=str(exc))
            if auto_fp_threshold and auto_fp_threshold > 0:
                try:
                    ...
                    logger.error('kb_plate_job_auto_fp_failed', error=str(exc))
            ...
            error = str(exc)
            logger.error('kb_plate_cluster_job_failed', error=str(exc))
        finally:
            _write_region_cluster_job(
                {
                    'running': False,
...
async def build_region_fp_centroids(client: AsyncOpenSearch) -> dict[str, Any]:
    ...
    if scroll_id:
        try:
            ...
            logger.warning('kb_fp_centroid_clear_scroll_failed', error=str(exc))
    ...
    try:
        ...
        logger.debug('kb_fp_centroid_refresh_failed', error=str(exc))
    ...
    logger.info('kb_build_plate_fp_centroids_done', n_members=n, k=int(k))
...
async def auto_assign_fp_from_centroids(
    ...
    if scroll_id:
        try:
            ...
            logger.warning('kb_auto_fp_clear_scroll_failed', error=str(exc))
    ...
    if moved:
        try:
            ...
            logger.debug('kb_auto_fp_refresh_failed', error=str(exc))

    logger.info(
        'kb_auto_assign_fp_done', n_scanned=n_scanned, n_moved=len(moved), threshold=threshold
    )
    return {
        'status': 'success',
        'n_scanned': n_scanned,
...
def _write_region_fp_job(state: dict[str, Any]) -> None:
    try:
        ...
        logger.warning('kb_plate_fp_job_write_failed', error=str(exc))
...
async def start_region_fp_centroid_job(client: AsyncOpenSearch) -> dict[str, Any]:
    ...
    async def _run() -> None:
        ...
        try:
            ...
            error = str(exc)
            logger.error('kb_plate_fp_job_failed', error=str(exc))
        finally:
            _write_region_fp_job(
                {
                    'running': False,
