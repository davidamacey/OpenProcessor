...
def detect_cluster_backend() -> BackendInfo:
    ...
    try:
        ...
        logger.info('kb_cluster_backend_cuda_unavailable', error=str(exc))
        return BackendInfo(
            name='cpu',
            detail=_cpu_detail(),
            free_vram_mb=None,
    ...
    try:
        ...
        logger.warning('kb_cluster_backend_memgetinfo_failed', error=str(exc))
        return BackendInfo(
            name='cpu',
            detail=_cpu_detail(),
            free_vram_mb=None,
    ...
    if free_mb < int(MIN_FREE_VRAM_GB_NN_DESCENT * 1024):
        logger.info(
            'kb_cluster_backend_low_vram_fallback',
            free_mb=free_mb,
...
def free_gpu_blocks() -> None:
    ...
    try:
        ...
        logger.debug('kb_cluster_backend_free_blocks_failed', error=str(exc))
