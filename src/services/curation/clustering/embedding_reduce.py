...
3. Persist the fitted reducer in two places (local joblib + OpenSearch
   ``kb_umap_state`` index) so a fresh container or a refit-less rerun
    ...
    * **Broad (True)** — same, but candidate-bucketed crops are also
    ...
    if progress is not None:
        try:
            ...
            logger.debug('kb_embedding_count_failed', error=str(exc))
    ...
    try:
        ...
        if scroll_id:
            try:
                ...
                logger.debug('kb_embedding_clear_scroll_failed', error=str(exc))
...
async def fetch_residual_v6_embeddings_parallel(
    ...
    *,
    ...
    try:
        ...
        logger.warning('kb_embedding_pit_unavailable_fallback_scroll', error=str(exc))
    ...
    try:
        ...
        try:
            ...
            logger.debug('kb_embedding_pit_delete_failed', error=str(exc))
...
def _load_umap_state_from_disk(path: str) -> Any | None:
    ...
    try:
        ...
        # the new container.
        logger.warning('kb_umap_state_disk_load_failed', path=path, error=str(exc))
        return None
...
async def _save_umap_state_to_opensearch(
    ...
    if len(blob) > _OPENSEARCH_PERSIST_MAX_BYTES:
        logger.info(
            'kb_umap_state_skip_opensearch_persist',
...
async def _load_umap_state_from_opensearch(client: AsyncOpenSearch, *, state_id: str) -> Any | None:
    try:
        ...
        logger.debug('kb_umap_state_not_found', state_id=state_id, error=str(exc))
        return None
    ...
    try:
        ...
        logger.warning('kb_umap_state_os_load_failed', state_id=state_id, error=str(exc))
        return None
...
def _to_numpy_float32(arr: Any) -> np.ndarray:
    ...
    for meth in ('get', 'to_numpy'):
        ...
        try:
            ...
            logger.debug('kb_to_numpy_method_failed', method=meth, error=str(exc))
    ...
    *,
    ...
    if reducer is None or mode == 'refit':
        if backend.name == 'gpu' and backend.build_algo is not None:
            ...
            logger.info('kb_umap_fit_cpu', n=len(embeddings))
        try:
        ...
        try:
            ...
            logger.warning('kb_umap_state_persist_failed', error=str(exc))
        refit_happened = True
    else:
        try:
