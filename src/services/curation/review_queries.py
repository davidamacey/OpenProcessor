...
def build_tab_query(
    ...
    *,
    ...
    behavior ``kb_review.py`` had inline before this split.
    """
    ...
    if tab == 'all':
        ...
        # Matches the v6_confidence_skip_gemma=0.80 default in kb_pipeline
        ...
        must_not.append({'term': {f'{fields.status}.keyword': 'no_plate_visible'}})
        must_not.append({'term': {f'{fields.status}.keyword': 'verify_rejected'}})
        ...
        must_not.append({'term': {f'{fields.status}.keyword': 'false_positive'}})
        ...
        must.append(
            {
                'script': {
                    'script': {
                        ...
                        # kb_clusters.py's top_class agg for the full story
