...
Directory resolved lazily via ``KB_SELECT_JOBS_DIR`` (default
...
embeddings and writes its own job-state file under ``KB_SELECT_JOBS_DIR``
...
def _jobs_dir() -> Path:
    return Path(os.environ.get('KB_SELECT_JOBS_DIR', '/jobs/select'))
...
async def run_selection_job(
    ...
    ``KB_SELECT_MAX_N``, only by the generous job-path ``max_n`` ceiling),
