...
def _build_reducer() -> Any:
    """Lazy ``import umap`` -- mirrors ``kb_embedding_reduce._build_cpu_reducer``
...
def _save_reducer_to_disk(reducer: Any) -> None:
    ...
    try:
        ...
        logger.warning('kb_umap_viz_state_disk_save_failed', error=str(exc))
...
async def _save_run_metadata(
    ...
    *,
    ...
    """Own metadata slot (``kb_umap_viz_state``, state_id='current') --
    distinct from the retired clustering reducer's ``kb_umap_state`` index.
    Metadata only (no pickled reducer blob) so this never risks the 100 MB
    OpenSearch request-body ceiling ``kb_embedding_reduce.py`` had to work
    ...
    try:
        ...
        logger.warning('kb_umap_viz_state_metadata_save_failed', error=str(exc))
...
async def _load_run_metadata(opensearch: AsyncOpenSearch) -> dict[str, Any] | None:
    try:
        ...
        logger.debug('kb_umap_viz_state_metadata_not_found', error=str(exc))
        return None
...
async def fit_projection(embeddings: np.ndarray) -> tuple[np.ndarray, str]:
    ...
    docstring point 2) -- never from ``kb_viz.py``'s GET handler."""
...
async def run_projection_job(
    ...
    *,
    ...
    try:
        ...
        logger.error('kb_viz_projection_job_failed', job_id=job_id, error=str(exc))
        state.status = 'failed'
        state.error = str(exc)
        state.finished_at = time.time()
        _atomic_write(state)
...
async def get_cached_projection(
    ...
    *,
    ...
    ``/kb/viz/projection`` contract (module docstring point 2).
    ...
    "partial coverage is visible" philosophy ``/kb/scores/coverage`` uses.
    """
    ...
    try:
        ...
        logger.warning('kb_viz_projection_staleness_check_failed', error=str(exc))
