...
Directory resolved lazily via ``KB_SCORES_STATE_DIR`` (default ``/jobs/scores``)
...
ONCE via :func:`kb_embedding_reduce.fetch_residual_v6_embeddings_parallel`
...
``class_validated=true`` at freeze time; see ``kb_review.freeze_test_holdout``).
"""
...
def _state_dir() -> Path:
    ...
    return Path(os.environ.get('KB_SCORES_STATE_DIR', '/jobs/scores'))
...
async def run_scoring_job(
    ...
    try:
        ...
        logger.error('kb_scores_job_failed', job_id=job_id, error=str(exc))
        state.status = 'failed'
        state.error = str(exc)
        state.finished_at = time.time()
        _atomic_write(state)
...
async def compute_coverage(opensearch: AsyncOpenSearch) -> dict[str, Any]:
    ...
    for scorer_id, meta in SCORER_METADATA.items():
        ...
        try:
            ...
            logger.warning('kb_scores_coverage_count_failed', scorer=scorer_id, error=str(exc))
            n = 0
        coverage[scorer_id] = {
            'field': field_name,
            'n_scored': n,
