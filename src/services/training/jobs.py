...
def _resolve_jobs_dir() -> Path:
    ...
    ``KB_TRAIN_JOBS_DIR`` with monkeypatch / env-var without re-importing.
    """
    return Path(os.environ.get('KB_TRAIN_JOBS_DIR', '/jobs'))
...
def _pin_registry_snapshot_sync(job_id: str) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        logger.warning('kb_train_registry_pin_read_failed', job_id=job_id, error=str(exc))
        return None, None
    ...
    try:
        ...
        logger.warning(
            'kb_train_registry_pin_write_failed',
            job_id=job_id,
...
async def write_cancel(job_id: str) -> None:
    ...
    await asyncio.to_thread(_do)
    logger.info('kb_train_cancel_written', job_id=job_id, path=str(target))
...
def _maybe_lost(status: TrainJobStatus) -> TrainJobStatus:
    ...
    try:
        ...
        logger.debug('kb_train_heartbeat_unparseable', job_id=status.job_id)
        return status
    ...
    for path, raw, kind in rows:
        if kind == 'status':
            try:
                ...
                logger.warning('kb_train_status_invalid', error=str(exc))
                continue
...
async def tail_run_log(job_id: str, lines: int = 200) -> list[str]:
    ...
    def _do() -> list[str]:
        ...
        try:
            ...
            logger.warning('kb_train_log_read_failed', job_id=job_id, error=str(exc))
            return []
...
async def cancel_campaign(campaign_id: str) -> int:
    ...
    for run in runs:
        ...
        cancelled += 1
    logger.info('kb_train_campaign_cancelled', campaign_id=campaign_id, n=cancelled)
    return cancelled
