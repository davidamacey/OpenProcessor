...
``scripts/curation/vlm_worker.py``'s ``main()``), raises ``RuntimeError``
naming the replacement for every retired name still set.
...
# previous wave (W1: the OP_VLM_* renames) did not already retire.
...
def reject_retired_env() -> None:
    ...
    if not stale:
        return
    renames = '; '.join(f'{name} was renamed to {RETIRED_ENV[name]}' for name in stale)
